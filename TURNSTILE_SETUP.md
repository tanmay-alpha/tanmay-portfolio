...
- When both `NEXT_PUBLIC_TURNSTILE_SITE_KEY` and `TURNSTILE_SECRET`
  are set, `POST /api/contact` requires a `turnstileToken` field.
  The server calls Cloudflare's `siteverify` endpoint, bound to the
  client's IP, with a 5 second timeout. A failed verification returns
  403.
- If only one Turnstile env var is set, the server returns 503 with
  the mailto fallback instead of accepting an unverified captcha or
  requiring a token the UI cannot mint.
- When `NEXT_PUBLIC_TURNSTILE_SITE_KEY` is set, the contact form
  renders the Turnstile widget. The submit button stays usable so a
  blocked captcha script can surface the email fallback instead of
  making the form look inert.
- The script is loaded on demand from
  `https://challenges.cloudflare.com/turnstile/v0/api.js`. CSP
  `script-src`, `connect-src`, and `frame-src` automatically allow
  that origin when `NEXT_PUBLIC_TURNSTILE_SITE_KEY` is set — you
  don't need to edit `next.config.js`.
